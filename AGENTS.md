...
- `config_reader.py` - Panel configuration reader via FLEXI_CFG mass storage. Discovers block device via sysfs (same USB parent), reads sectors after authenticated export trigger, XOR 0xFF decrypts, parses section names and device entries (name, section, rf_byte0).
...
- `coordinator.py` - DataUpdateCoordinator combining push and poll. Section states and device activity pushed by panel via reader thread. Device status (battery, signal, voltage) polled at configurable interval (default 30 min) via `_async_update_data`. Triggers reauth on unexpected WRONG_CODE.
